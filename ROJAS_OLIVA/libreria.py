...
def pedir_booleano(num):
    boll=""
    while(validar_booleano(boll)== False):
        boll=input(msg)
        return boll
...
#funcion que valida si el dia del mes es valido
#parametro: dia(str)
def validar_dia(dia):
    #1. verificar si la cadena tiene forma de digito
    #2. la longitud debe ser 2
    #3. debe ser >0 y <31
    if(dia.isdigit()==True):
        if(len(dia)==2):
            dia=int(dia)
...
#funcion que valida si el mes del año es valido
def validar_mes(mes):
    #1. verificar si la cadena tiene forma de digito
    #2. la longitud debe ser 2
    #3. debe ser >0 y <13
...
def validar_fecha(fecha):
    #1. la fecha debe tener una longitud de 10
    #            0123456789
    #2. formato: YYYY-MM-DD
    #3. YYYY es un año valido
    #4. MM es un mes  valido
    #5. DD es un dia valido
    if(len(fecha)==10):
        if(fecha[4]=="-" and fecha[7]=="-"):
            anio=fecha[0:4]
            mes=fecha[5:7]
            dia=fecha[8:]
            if(validar_anio(anio)==True and validar_mes(mes)==True and validar_dia(dia)==True):
                return True
            else:
                return False
        else:
